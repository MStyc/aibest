...
export default function ChatPage() {
  ...
  const handleGroupResponse = async (prompt: string, currentMessages: any[]) => {
    ...
    try {
      ...
      const typingDelay = calculateTypingDelay(response?.length ?? 20)

      setTimeout(() => {
        setIsTyping(false)
        const aiMessage = {
          role: "model",
          content: { type: "text", content: response, senderName: respondingMember.name },
        }
    ...
    <div className="h-screen w-full bg-black flex overflow-hidden">
      ...
      {allProfiles.length > 0 && (
        <div
          ...
          ${
            isMobile
              ? `fixed inset-0 z-50 transform transition-transform duration-300 ${showInbox ? "translate-x-0" : "-translate-x-full"}`
              : "w-80 flex-shrink-0"
          }
        `}
        >
      ...
      <div className={`flex-1 flex flex-col ${activeProfile.theme || "bg-animated-blue-purple"}`}>
        ...
        <div className="chat-header flex items-center justify-between p-4 bg-black/90 backdrop-blur-sm border-b border-white/10">
          <div className="flex items-center">
            ...
            <img
              src={activeProfile.img || `https://placehold.co/32x32/667eea/ffffff?text=${activeProfile.name.charAt(0)}`}
              alt={activeProfile.name}
              className="w-8 h-8 rounded-full object-cover mr-3"
            />
            <div>
              <h2 className="text-white font-semibold text-lg">{activeProfile.name}</h2>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
                  ...
                  src={
                    activeProfile.type === "group" && message.content.senderName
                      ? activeProfile.members?.find((m) => m.name === message.content.senderName)?.img ||
                        `https://placehold.co/28x28/667eea/ffffff?text=${message.content.senderName.charAt(0)}`
                      : activeProfile.img ||
                        `https://placehold.co/28x28/667eea/ffffff?text=${activeProfile.name.charAt(0)}`
                  }
          ...
          {isTyping && (
            <div className="flex items-end space-x-2 justify-start">
              <img
                src={
                  activeProfile.img || `https://placehold.co/28x28/667eea/ffffff?text=${activeProfile.name.charAt(0)}`
                }
                alt={activeProfile.name}
                className="w-7 h-7 rounded-full object-cover flex-shrink-0"
              />
      ...
      {/* Mobile overlay */}
      {isMobile && showInbox && (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-40" onClick={() => setShowInbox(false)} />
      )}
    </div>
  )
}
